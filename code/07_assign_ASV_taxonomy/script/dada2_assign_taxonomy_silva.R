...
# assign taxa
taxa <- assignTaxonomy(seqs=asv.id.table$ASV, "~/tax/dada2/silva_v138/silva_nr99_v138.1_train_set.fa.gz", multithread=TRUE)

# Add species.
taxaS <- addSpecies(taxa, "~/tax/dada2/silva_v138/silva_species_assignment_v138.1.fa.gz")
...
# Save taxa info.
write.table(asvTaxa, 
            file=file.path(outDir, "asvTaxaTable_silva-v138.txt"), 
...
writeLines(lines, file.path(outDir, "summary_silva-v138.txt"))
