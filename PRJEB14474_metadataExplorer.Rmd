...
```{r}
data = read.delim2("PRJEB14474_SraRunTable.txt")
row.names(data) = data$ID
```

Save any output to this folder:
```{r}
outDir = "PRJEB14474_output"
...
df2 = df[,c("ID", "Sample.Name", "Sample_name", "host_subject_id", "Date", "study_day", "SAMPLE_TYPE", "Location", "run_prefix..exp.", "env_material", "surcat")]
```
...
hm = ggplot(df7, aes(x=Location, y=SAMPLE_TYPE, color=n, fill=n)) + 
  ...
  ggtitle("PRJEB14474 samples", subtitle = paste("N =", nrow(data)))
hm

ggsave(file.path(outDir, "Locatoin-sampletype_heat.png"), hm)
write.table(df2 %>% 
              count(Location, SAMPLE_TYPE) %>% 
...
```{r}
df4 = df2 %>% mutate(ishDay=round(study_day, -1)) %>% count(ishDay, SAMPLE_TYPE)
...
```
Yes! surcat is interesting because it distinguishes staff skin from patient skinn for "Hand" and "Nose" samples. (that was implied by location...but not explicit)
