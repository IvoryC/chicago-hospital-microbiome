...
In an individual email correspondence, I asked the authors for patient IDs.  They generously sent a mapping file. The "SampleID" in that file, corresponds to the second half of the "Library Name" column in this metadata.  So keep that around. 

Unfortunately, using that, I found that some values that should have been "1000" were represented as "1", it looks like the issue is somewhere upstream of the sra metadata. Fortunately, the "Submitter_Id" field has a qiita identifier, that also includes this id and it has the "1000" value. So just use that instead.
...
We see `r nrow(data)` described in the sra metadata, which comes in as `r sum(m3$sra.total)` in the joined table.  We are missing `r sum(m3$missing.tot)` samples.
